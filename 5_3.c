...
void web3func(web2param*param){
    ///puts(param->buffer);
    (void) write_with_count_time(param->fd, param->buffer, param->len,param->times,WRITE_SOCKET);
    close(param->fd);
    free(param->buffer);
    free(param);
}
...
void web2func(web2param*param){
    int ret;
    ret = read_with_count_time(param->file_fd, param->buffer, param->len,param->times,READ_LOG);
    param->len = ret;
...
/* 根据预定义在 extensions 中的文件类型,检查请求的文件类型是否本服务器支持 */
        ...
        if(file_fd!=-1) {
            ...
            para->times = times;
            para->len = len;
            para->buffer = malloc(sizeof(char) * len);
        }
