...
int mmap_read(int fd,char *buf,int len){
    int tot = len* sizeof(char);
    tot = ((tot%PAGE_SIZE)!=0)+tot/PAGE_SIZE;
    tot*=PAGE_SIZE;
    char *fp = mmap(NULL, tot, PROT_READ, MAP_SHARED, fd, 0);
    if(fp!=((void*)(-1))) {
        memcpy(buf, fp, len * sizeof(char));
        munmap(fp, tot);
        buf[len] = 0;
    }else {
...
int read_web_log_with_count_time(int fd,char *buffer,int BufferSize,double *Time,enum TIME_IN_COUNT POS){
    struct timeval t1,t2;
    int ret = -1;
    gettimeofday(&t1,NULL);
    if(BufferSize>10*PAGE_SIZE) {
        ret = mmap_read(fd, buffer, BufferSize);
    }else ret = read(fd,buffer,BufferSize);
