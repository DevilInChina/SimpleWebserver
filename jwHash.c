...
HASHRESULT del_by_str( jwHashTable *table, char *key,void (*destroyFunc)(void*))
{
    // compute hash on key
    size_t hash = hashString(key) % table->buckets;
    ...
    while(entry!=0)
    {
        ...
        if(0==strcmp(entry->key.strValue,key)) {
            ...
            switch (entry->valtag){
                case HASHPTR:
                    destroyFunc(entry->value.ptrValue);
                    break;
                case HASHSTRING:
                    free(entry->value.strValue);
                    break;
...
// Delete by int
HASHRESULT del_by_int( jwHashTable* table, long int key ,void (*destroyFunc)(void*))
{
	// compute hash on key
	size_t hash = hashInt(key) % table->buckets;
