...
void web3func(web3param*param){
    ...
    while ((ret = read_with_count_time(param->file_fd, param->buffer, BUFSIZE,param->times,READ_LOG)) > 0) {
        (void) write_with_count_time(param->fd, param->buffer, ret,param->times,WRITE_SOCKET);
    }
    close(param->file_fd);
    close(param->fd);
    free(param);
...
/* 根据预定义在 extensions 中的文件类型,检查请求的文件类型是否本服务器支持 */
       ...
       // usleep(10);
        ...
        web3param * para = malloc(sizeof(web3param));
        para->file_fd = file_fd;
        para->fd = fd;
        para->times = times;
        thpoolAddWork(web3,(void*)web3func,para);
